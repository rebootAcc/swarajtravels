...
export default function AddNewRentalForm({
  ...
  const addNewRental = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (seasonPrice === "") {
      setErrors((prev) => ({ ...prev, seasonPrice: true }));
      return;
    }
    if (offSeasonPrice === "") {
      ...
      <div className="grid grid-cols-2 gap-8">
        <div>
          <select
            className={`h-[4rem] px-2 xl:px-7 bg-[#f5f5f5] w-full rounded-md placeholder-typeograph-2 text-base text-typeograph-1 outline-none ${
              errors.type ? "border border-red-600" : "border-0"
            }`}
            value={type}
            onChange={(e) => {
        ...
        <div>
          <input
            type="number"
            className={`h-[4rem] px-2 xl:px-7 bg-[#f5f5f5] w-full rounded-md placeholder-typeograph-2 text-base text-typeograph-1 outline-none ${
              errors.seasonPrice ? "border border-red-600" : "border-0"
            }`}
            value={seasonPrice}
            onChange={(e) => {
              setErrors((prev) => ({ ...prev, seasonPrice: false }));
              setSeasonPrice(e.target.value);
            }}
        ...
        <div>
          <input
            type="number"
            className={`h-[4rem] px-2 xl:px-7 bg-[#f5f5f5] w-full rounded-md placeholder-typeograph-2 text-base text-typeograph-1 outline-none ${
              errors.offSeasonPrice ? "border border-red-600" : "border-0"
            }`}
            value={offSeasonPrice}
            onChange={(e) => {
              setErrors((prev) => ({ ...prev, offSeasonPrice: false }));
              setOffSeasonPrice(e.target.value);
            }}
        ...
        <div className="h-[4rem] px-2 xl:px-7 bg-[#f5f5f5] w-full rounded-md placeholder-typeograph-2 text-base text-typeograph-1 outline-none flex items-center">
          ...
          >
            ...
            <span>
              {uploadedPicture1 ? uploadedPicture1.name : "upload picture"}
            </span>
          </label>
        </div>
      </div>
