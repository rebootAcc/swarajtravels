...
import RentalsTable from "@/features/RentalsTable";
import AdminDashboardTemplate from "@/template/AdminDashboardTemplate";
import Link from "next/link";

async function getALLRentals() {
  const response = await fetch(process.env.API_URI + "/api/rentals/");
  const pageData = await response.json();
  return pageData;
}

export default async function Rentals() {
  const pageData = await getALLRentals();
  ...
  return (
    <AdminDashboardTemplate>
      ...
      <div className="xlg:px-8 px-4 py-4 flex flex-col gap-2">
        <RentalsTable tableData={pageData} />
        <div className="flex justify-between py-4 text-typeograph-1 text-sm lg:text-base">
          <span>
            Showing {pageData.rentals.length} of{" "}
            {pageData.pagination.totalRentals} rental
          </span>
          <div>
